...
describe("A user", () => {
    ...
    describe("with a trydotnet session", () => {
        it("can open a document", async () => {
            ...
            expect(document.id().toString()).to.equal("program.cs");
            expect(document.getContent()).to.equal("file content");
        });
        ...
        it("creates a empty document when the project does not have a matching file", async () => {
            ...
            expect(document.id().toString()).to.equal("program_two.cs");
            expect(document.getContent()).to.equal("");
        });
        ...
        it("creates a empty document when using region and the project does not have a matching file", async () => {
            ...
            expect(document).to.not.be.null;
            expect(document.id().toString()).to.equal("program_two.cs@controller");
            expect(document.getContent()).to.equal("");
        });
        ...
        it("can open a document with region as identifier", async () => {
            ...
            expect(document.id().toString()).to.equal("program.cs@controller");
            expect(document.getContent()).to.equal("//content");
        });
        ...
        it("can open a document and bind it immediately to an editor", async () => {
            let editorState = { content: "", documentId: undefined as unknown as DocumentId };
            let session = await createReadySession(configuration, editorIFrame, dom.window);
            let project = await createProject({ packageName: "console", files: [{ name: "program.cs", content: "file content" }] });
            ...
            await wait(1000);
            expect(editorState.content).to.equal("file content");
            expect(editorState.documentId.toString()).to.equal("program.cs");
        });
        ...
        it("can return the open documents", async () => {
            ...
            expect(document.getContent()).to.equal("i am a document");
        });
        ...
        describe("and with a document", () => {
            it("can set the content and affect editor", async () => {
                ...
                expect(document.getContent()).to.equal("new content");
                await wait(1000);
                expect(editorState.content).to.equal("new content");
            });
            ...
            it("can track editor changes", async () => {
                ...
                await wait(1000);
                expect(document.getContent()).to.equal("new editor content");
            });
        });
    });
});